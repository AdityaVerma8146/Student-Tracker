import type { Subject, DailyTask, Statistics, WeekDate } from '../types'

export const storageKey = 'syllabusTrackerData'

export const calculateChapterCompletion = (chapter: { topics?: { completed: boolean }[] }): number => {
...
export const calculateSubjectCompletion = (subject: Subject): number => {
...
export const calculateOverallCompletion = (subjects: Subject[]): number => {
...
export const getMondayWeekStart = (referenceDate: Date = new Date()): Date => {
...
const countScheduleCompletions = (dailyTasks: DailyTask[], rangeStart: Date, rangeEnd: Date): number => {
...
export const getProgressByDays = (dailyTasks: DailyTask[], days = 7) => {
...
const getWeekRanges = (weeks = 4): DateRangeEntry[] => {
  const today = new Date()
  const result: DateRangeEntry[] = []
...
export const getProgressByWeeks = (dailyTasks: DailyTask[], weeks = 4) => {
...
const getMonthRanges = (months = 6): DateRangeEntry[] => {
  const today = new Date()
  const result: DateRangeEntry[] = []
...
export const getProgressByMonths = (dailyTasks: DailyTask[], months = 6) => {
...
export const exportData = (data: unknown): void => {
...
export const importData = (file: File): Promise<{ subjects: Subject[]; [key: string]: unknown }> => {
