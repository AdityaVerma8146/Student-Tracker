    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 card-shadow">
      ...
      <form onSubmit={handleAdd} className="mt-6 grid gap-3 sm:grid-cols-[2fr_1fr_auto]">
        ...
        <button
          type="submit"
          onClick={spawnRipple}
          className="ripple-container btn-primary inline-flex items-center justify-center gap-2 rounded-lg px-5 py-3"
        >
                  ...
                  {weekDates.map(day => (
                    <td key={day.key} className="py-4 text-center align-top">
                      <button
                        ...
                        className={`inline-flex items-center justify-center w-9 h-9 rounded-full border transition-colors ${schedule[day.key] ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 dark:border-gray-700 text-gray-500 hover:border-blue-500 dark:hover:border-blue-400'}`}
                        title={`${day.label} ${schedule[day.key] ? 'completed' : 'not completed'}`}
                      >
                        <span key={schedule[day.key] ? 'on' : 'off'} className={schedule[day.key] ? 'check-pop flex' : 'flex'}>
