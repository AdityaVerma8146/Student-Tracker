...
import { Heart, Smile, Meh, Frown, Pencil, Trash2, Check, X, BookMarked } from 'lucide-react'
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-6">
        <h1 className="text-4xl font-bold flex items-center gap-2"><BookMarked size={30} className="text-blue-600 dark:text-blue-400" /> My Diary</h1>
                ...
                {moodOptions.map((mood) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center gap-2 rounded-2xl border px-4 py-2 transition ${selectedMood === mood.emoji ? 'border-blue-600 bg-blue-50 text-blue-700 dark:bg-[rgba(var(--accent-rgb),0.1)] dark:border-blue-400' : 'border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200'}`}
                    >
                      <Icon size={18} />
                        ...
                        <div className="flex flex-wrap gap-2">
                          {moodOptions.map((mood) => (
                            <button
                              ...
                              className={`rounded-xl border px-3 py-1.5 text-sm transition ${editMood === mood.emoji ? 'border-blue-600 bg-blue-50 text-blue-700 dark:bg-[rgba(var(--accent-rgb),0.1)] dark:border-blue-400' : 'border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200'}`}
                            >
                              {mood.emoji}
                            </button>
                          ))}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                              <button
                                ...
                                title="Edit entry"
                                className="p-1.5 rounded-lg text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:text-gray-400 dark:hover:bg-[rgba(var(--accent-rgb),0.1)] dark:hover:text-blue-400 transition"
                              >
                                <Pencil size={15} />
                              </button>
                              <button
